...
if(isset($_GET['p_id']) && $_GET['p_id'] != ""){
    //使用p_id產品代碼取出資料
    //處理第一層
    $sql = sprintf("SELECT *FROM product, pyclass, (SELECT classid as upclassid, level as uplevel, cname as upcname FROM pyclass WHERE level=1) as uplevel WHERE product.classid=pyclass.classid AND pyclass.uplink=uplevel.upclassid AND product.p_id=%d", $_GET['p_id']);
    ...
    //列出產品類別查詢
    $sql = sprintf("SELECT * FROM pyclass WHERE level=%d AND classid=%d", $_GET['level'], $_GET['classid']);
    $classid_rs = mysqli_query($link, $sql);
    $data = mysqli_fetch_array($classid_rs);
    ...
    //選擇第二層類別
    $sql = sprintf("SELECT * FROM pyclass WHERE level=2 AND classid=%d", $_GET['classid']);
    $classid_rs = mysqli_query($link, $sql);
    $data = mysqli_fetch_array($classid_rs);
    $level2Cname = $data['cname'];
    ...
    //需加處理上一層
    $sql = sprintf("SELECT * FROM pyclass WHERE level=1 AND classid=%d", $level2Uplink);
    $classid_rs = mysqli_query($link, $sql);
    $data = mysqli_fetch_array($classid_rs);
    $level1Cname = $data['cname'];
    $level1 = $data['level'];
